...
* The second argument is mis-matched to `b` (well, you can't tell that from the
  above, but trust me, it is).
...
is that `match.call` always uses the context of the function that invokes
...
be matching the dots in the parent context of `fun_par`, instead of the parent
context of `fun_gpar`.

Since `fun_gpar` has a formal argument `b` in addition to `...`, when we invoke
`fun_gpar(...)` the actual `...` argument inside `fun_gpar` will be the original
`...` argument passed to `fun_gpar`, _less_ the explicit formal arguments that
are matched.  In this case, `b` positionally matches the 999, so the `...` inside 
`fun_gpar` no longer contains 999, but `match.call` uses this 999-less `...` to 
match the `fun_gpar(...)` call, not the original `...` in the parent context of 
`fun_gpar`.

This behavior is not unreasonable since `match.call` doesn't provide any way to
give it the correct context for substituting the dots in.
...
note here we make `fun_par` an anonymous function for the sake of compactness
...
# fun_gpar(b = a, c = 58, ... = list(x = 45, zest = "lemon"))
...
# fun_gpar(b = a, c = ..3, d = ..4, ... = list(x = 45, zest = "lemon"))
